...
@onready var DebugText1 = $Counters/Text/Node2D/DebugNumbers_1
@onready var DebugText2 = $Counters/Text/Node2D/DebugNumbers_2
...
@export var act = 1
@export var waterpalettedry = preload("res://Graphics/Palettes/BaseHPZ.png")
@export var waterpalettewet = preload("res://Graphics/Palettes/WetHPZ.png")
var WaterPal = preload("res://Shaders/WaterPalette.tres")
...
func _ready():
	...
	#$Water/WaterOverlay.set_shader_parameter("originalPalette",waterpalettedry)
	#$Water/WaterOverlay.set_shader_parameter("swapPalette",waterpalettewet)
	WaterPal.set_shader_parameter("originalPalette",waterpalettedry)
	WaterPal.set_shader_parameter("swapPalette",waterpalettewet)
	...
	if playLevelCard:
		...
		$LevelCardS2/S2_CardMover.play("End")
...
func _process(delta):
	...
	if Global.debug_mode == true:
		$Counters/Text/Node2D/DebugNumbers_1.visible = true
		$Counters/Text/Node2D/DebugNumbers_2.visible = true
		$Counters/Text/ScoreNumber.visible = false
		DebugText1.text = ("%1d" % Global.players[focusPlayer].global_position.x).pad_zeros(4) + " " + ("%1d" % Global.players[focusPlayer].global_position.y).pad_zeros(4)
		DebugText2.text = ("%1d" % abs(Global.players[focusPlayer].groundSpeed)).pad_zeros(4) + " " + ("%1d" % Global.players[focusPlayer].enemyCounter).pad_zeros(2) + ("%1d" % Global.players[focusPlayer].collissionLayer).pad_zeros(1)+ ("%1d" % Global.players[focusPlayer].z_index).pad_zeros(1)
	else:
		$Counters/Text/Node2D/DebugNumbers_1.visible = false
		$Counters/Text/Node2D/DebugNumbers_2.visible = false
	...
	if Global.stageClearPhase > 2:
		...
		if !isStageEnding:
			...
			$LevelClear/ClearText/ScoreText/ScoreNumber.text = scoreText.text
			$LevelClear/Animator.play("LevelClear")
			
			# set bonuses
			ringBonus = floor(Global.players[focusPlayer].rings)*100
			$LevelClear/ClearText/RingBonus/RingNumbers.text = "%6d" % ringBonus
			timeBonus = 0
			# bonus time table
			var bonusTable = [
			[60*5,500],
			...
			$LevelClear/ClearText/TimeBonus/TimeNumbers.text = "%6d" % timeBonus
