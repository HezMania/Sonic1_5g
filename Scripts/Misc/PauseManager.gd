...
enum MENUS {MAIN, OPTIONS, LEVELSELECT, RESTART, QUIT}
var option = 0
			...
			MENUS.OPTIONS: # options menu
				match(option): # Options
					...
					5: # back
						Global.save_settings()
						set_menu(0)
			MENUS.LEVELSELECT: # Level select go to
				match(option): # Options
					0: # cancel
						set_menu(0)
					1: # ok
						...
						#Global.main.change_scene_to_file(load("res://Scene/Presentation/LevelSelect.tscn"),"WhiteOut","WhiteOut",1,true,false)
						Global.nextZone = load("res://Scene/Presentation/LevelSelect.tscn")
