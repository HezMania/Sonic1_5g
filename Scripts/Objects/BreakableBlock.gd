...
@export var Art = preload("res://Graphics/Obstacles/Blocks/breakable_block.png")
@export var pieces = Vector2(2,2)
@export var piecspeed = Vector2(1,1)
...
func _process(delta):
	$BlockSprite.texture = Art
	$CollisionShape2D.scale = pieces
	pass
...
func physics_collision(body, hitVector):
	...
	if hitVector == Vector2.DOWN and body.get_collision_layer_value(20):
		...
		# and set the settings for each piece to match up with the $BlockSprite node
		for i in range(pieces.x):
			for j in range (pieces.y):
				var piece = Piece.instantiate()
				
				piece.velocity = Vector2(
				(pieces.y-j)*lerp(-piecspeed.x,piecspeed.x,i/(pieces.x-1)),
				-pieces.y+j)*(60*piecspeed.y)
				
				var spriteWidth = $BlockSprite.texture.get_width()
				var spriteHeight = $BlockSprite.texture.get_height()
				if $BlockSprite.region_enabled:
					spriteWidth = $BlockSprite.region_rect.size.x
					spriteHeight = $BlockSprite.region_rect.size.y
				
				piece.global_position = global_position+Vector2(
				spriteWidth/4*lerp(-1,1,i/(pieces.x-1)),
				spriteHeight/4*lerp(-1,1,j/(pieces.y-1))
				)
				piece.texture = $BlockSprite.texture
