...
func _on_PlayerCheck_body_entered(_body):
	if !isFiring and coolDown <= 0:
		...
		# set sprites to 
		$Sprite2D/BatBomberBody.play("Shooting")
		...
		fire.global_positionc = gclobal_position+Vector2(25*side,25)
		fire.scale.x = -side
		...
		$Sprite2D/BatBomberBody.play("Normal")
