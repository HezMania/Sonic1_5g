...
var stateTimer = 0
var animTime = 0
@export var speed_normal = 30
@export var speed_chasing = 45
...
func _physics_process(delta):
	# Dirction checks
	$Crocobot.scale.x = abs($Crocobot.scale.x)*-direction
	$PlayerCheck.scale.x = abs($PlayerCheck.scale.x)*-direction
	...
	#  Set crocs mouth open and speed him up	
func _on_area_2d_body_entered(body):
	$Crocobot.play("MouthOpen")
	speed = speed_chasing

	#  Set crocs mouth closed and slow him back down
func _on_area_2d_body_exited(body):
	$Crocobot.play("Normal")
	speed = speed_normal
