...
SUPER, DEBUGMODE, FLY, RESPAWN, HANG, GLIDE, WALLCLIMB, AMYHAMMER}
...
@onready var debugSprite = $DebugMode
...
var Sonic1Animations = preload("res://Graphics/Players/PlayerAnimations/Sonic1.tscn")
...
func _process(delta):
	...
	if (invTime > 0 and currentState != STATES.HIT and currentState != STATES.DIE and currentState != STATES.DEBUGMODE):
		visible = !visible
		invTime -= delta*60
		if (invTime <= 0):
			invTime = 0
			visible = true
	if (ringDisTime > 0) and (currentState != STATES.HIT or currentState != STATES.DEBUGMODE):
		ringDisTime -= delta
	...
	if water and shield != SHIELDS.BUBBLE:
		if airTimer > 0:
			...
			airTimer -= delta
		elif currentState != STATES.DIE or currentState != STATES.DEBUGMODE: # Drown (kill checks if air timer is greater then 0)
	...
	#DEBUG MODE ACTIVATE
	if inputs[INPUTS.ACTION2] == 1 and Global.debug_mode == true and playerControl == 1:
		action_debug()
		set_state(STATES.DEBUGMODE)
func _physics_process(delta):
	...
	# Water
	if Global.waterLevel != null and (currentState != STATES.DIE or currentState != STATES.DEBUGMODE):
		# Enter water
...
func hit_player(damagePoint = global_position, damageType = 0, soundID = 6):
	if damageType != 0 and shield == damageType+1:
		return false
	if ((currentState != STATES.HIT or currentState != STATES.DEBUGMODE) and invTime <= 0 and supTime <= 0 and (shieldSprite.get_node("InstaShieldHitbox/HitBox").disabled or character != Global.CHARACTERS.SONIC)):
...
func cam_update(forceMove = false):
	...
	# Camera lock
	# remove round() if you are not making a pixel perfect game (THISISWHERECAMERASTUFFIS)
...
func action_debug(animation = "idle", airJumpControl = true, playSound=true):
	if forceRoll <= 0: # check to prevent jumping in roll tubes
		animator.play(animation)
		animator.advance(0)
		movement.y = 0
