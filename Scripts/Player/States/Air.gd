...
func _process(_delta):
	if parent.playerControl != 0 or (parent.inputs[parent.INPUTS.YINPUT] < 0 and parent.character == Global.CHARACTERS.TAILS):
		# Super
		if parent.inputs[parent.INPUTS.ACTION] == 1 and !parent.isSuper and isJump:
...
func _physics_process(delta):
	...
	if (isJump):
		...
		if parent.character == Global.CHARACTERS.AMY: ## or parent.character == Global.CHARACTERS.SONIC:
