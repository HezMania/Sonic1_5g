...
func _process(delta):
	...
	#if parent.inputs[parent.INPUTS.ACTION2] == 1 and Global.debug_mode == true and parent.playerControl == 1:
		#parent.sfx[17].play()
		#parent.action_debug()
		#parent.set_state(parent.STATES.DEBUGMODE)
	# jumping / rolling and more (note, you'll want to adjust the other actions if your character does something different)
	if (parent.any_action_pressed() and Global.debug_mode == false) or (Global.debug_mode == true and (parent.inputs[parent.INPUTS.ACTION] == 1 or parent.inputs[parent.INPUTS.ACTION3] == 1)):
